...
minimizing the number of changes of direction required and staying below the maximum time and energy available.
...
class MissionPlanOptimizer:
    ...
    def __init__(self, time_matrix: List[List[int]], energy_matrix: List[List[int]], changes_dir_matrix: List[List[int]],
        ...
        :param changes_dir_matrix: list of lists (n+1)x(n+1) where E[i, j] is the number of changes of direction required to go from goal j to goal i
        ...
        """
        if not isinstance(time_matrix, List) or not isinstance(energy_matrix, List) or not isinstance(changes_dir_matrix, List)\
        ...
        self.__compute_from_to_lists__()
    ...
    def __compute_from_to_lists__(self):
        """
        Computes lists needed by predicate 'dreachable' of minizinc.
        """
        for i in range(self.N):
            for j in range(self.N):
                if j != i:
                    self.from_list.append(j+1)
                    self.to_list.append(i+1)
